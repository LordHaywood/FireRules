...
export const ValidPathObj = (path: RulesInternalFieldId): boolean =>
  path.length != 0 &&
    typeof path[0] != "number" &&
    !path.reduce((r, v) => r || (typeof v == "number" && (!Number.isInteger(v)) || v < 0), false);

export const ExtractPath = (pathStr: string): RulesInternalFieldId =>
  pathStr.split(/[\.[]/g).map(v => v[v.length - 1] == "]" ? parseInt(v.substr(0, v.length - 1)) : v);

export const PathStringify = (path: RulesInternalFieldId): string =>
